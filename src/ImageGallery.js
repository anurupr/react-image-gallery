...
function setNegativeIfNot(value) {
  if (value > 0) {
    return -value;
  }
  return value;
...
export default class ImageGallery extends React.Component {
  ...
  slideThumbnailBar(previousIndex) {
    ...
    const { items } = this.props;
    const lastScroll = this.getThumbsTranslate(items.length - 1);
    if (currentIndex === 0) {
      ...
      if (scroll > 0) {
        if (previousIndex < currentIndex) {
          const toScroll = thumbsTranslate - scroll;
          if (toScroll < lastScroll) {
            this.setThumbsTranslate(setNegativeIfNot(lastScroll));
          } else {
            this.setThumbsTranslate(setNegativeIfNot(toScroll));
          }
  ...
  handleThumbnailSwiping({ event, absX, dir }) {
    let thumbnailWrapperWidth = 0;

    if (this.thumbnailsWrapper && this.thumbnailsWrapper.current) {
      thumbnailWrapperWidth = this.thumbnailsWrapper.current.offsetWidth;
    }

    this.setScrollDirection(dir);
    event.stopPropagation();

    let currentSlideOffset = (absX / thumbnailWrapperWidth) * 100;
    if (Math.abs(currentSlideOffset) >= 100) {
      currentSlideOffset = 100;
    }
  }
  ...
  handleOnThumbnailSwiped({ event, dir, velocity }) {
    const { disableSwipe, stopPropagation } = this.props;
    const { scrollingUpDown, scrollingLeftRight, thumbsTranslate } = this.state;
    ...
    const scroll = -this.getThumbsTranslate(items.length - 1);

    const swipeDirection = dir === LEFT ? -1 : +1;
    // const isSwipeUpOrDown = dir === UP || dir === DOWN;
    const factor = velocity * 1000;
    let newThumbsTranslate = thumbsTranslate + factor * swipeDirection;

    if (newThumbsTranslate > 0) {
      newThumbsTranslate = 0;
    }

    if (newThumbsTranslate < scroll) {
      newThumbsTranslate = scroll;
    }

    this.setState({
      thumbsTranslate: newThumbsTranslate,
    });
  }
  ...
  handleResize() {
    const { items } = this.props;
    const { currentIndex, thumbnailsWrapperHeight, thumbnailsWrapperWidth } =
      this.state;
    ...
    let updateThumbsTranslate = false;
    ...
    if (updateThumbsTranslate) {
      const scroll = -this.getThumbsTranslate(items.length - 1);
      let newThumbsTranslate = -this.getThumbsTranslate(currentIndex);
      // Adjust thumbnail container when thumbnail width or height is adjusted
      if (newThumbsTranslate < scroll) {
        newThumbsTranslate = scroll;
      }

      this.setThumbsTranslate(newThumbsTranslate);
    }
  }
        ...
        <div className={igContentClass}>
          ...
          {showThumbnails && (
            <SwipeWrapper
              ...
              delta={0}
              onSwiping={this.handleThumbnailSwiping}
              onSwiped={this.handleOnThumbnailSwiped}
            >
              <div
